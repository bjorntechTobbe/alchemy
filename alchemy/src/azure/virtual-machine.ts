...
import type { VirtualMachine as AzureVirtualMachine } from "@azure/arm-compute";
import { isNotFoundError } from "./error.ts";
import { deleteWithAbortVerification } from "./retry.ts";
  ...
  async function (
    ...
    if (this.phase === "delete") {
      if (props.delete !== false) {
        // Delete VM first
        if (virtualMachineId) {
          await deleteWithAbortVerification(
            () =>
              clients.compute.virtualMachines.beginDeleteAndWait(
                resourceGroupName,
                name,
              ),
            () => clients.compute.virtualMachines.get(resourceGroupName, name),
        ...
        if (networkInterfaceId) {
          await deleteWithAbortVerification(
            () =>
              clients.network.networkInterfaces.beginDeleteAndWait(
                resourceGroupName,
                nicName,
              ),
            () =>
              clients.network.networkInterfaces.get(resourceGroupName, nicName),
        ...
        await deleteWithAbortVerification(
          () =>
            clients.compute.disks.beginDeleteAndWait(
              resourceGroupName,
              osDiskName,
            ),
          () => clients.compute.disks.get(resourceGroupName, osDiskName),
          `disk "${osDiskName}"`,
          () => {
    ...
    if (this.phase === "update" && this.output) {
      ...
      if (customDataHash !== this.output.customDataHash) {
        replaceReasons.push(
          `customData changed (hash: ${this.output.customDataHash} → ${customDataHash})`,
        );
      }
      ...
      if (this.output.osType !== (props.osType || "Linux")) {
        replaceReasons.push(
          `osType: ${this.output.osType} → ${props.osType || "Linux"}`,
        );
      }
      ...
      const currentVnet =
        typeof this.output.virtualNetwork === "string"
      ...
      if (this.output.subnetName !== props.subnetName) {
        replaceReasons.push(
          `subnetName: ${this.output.subnetName} → ${props.subnetName}`,
        );
      }

      if (replaceReasons.length > 0) {
        console.log(
          `[VirtualMachine] Replacing VM due to:\n  - ${replaceReasons.join("\n  - ")}`,
        );
        ...
        if (isNotFoundError(error)) {
          ...
          await clients.network.virtualNetworks.beginCreateOrUpdateAndWait(
            ...
            defaultVnetName,
            {
              ...
              subnets: [
                { name: defaultSubnetName, addressPrefix: "10.0.0.0/24" },
              ],
            },
          );
        } else {
    ...
    const vmParams: AzureVirtualMachine = {
      ...
      storageProfile: {
        ...
        osDisk: {
          ...
          managedDisk: {
            storageAccountType:
              props.osDiskStorageAccountType || "Standard_LRS",
          },
        },
      },
      osProfile: {
    ...
    if (existingVm) {
      ...
      if (
        props.customData &&
        this.output?.customDataHash &&
        customDataHash !== this.output.customDataHash
      ) {
        replaceReasons.push(
          `customData changed (hash: ${this.output.customDataHash} → ${customDataHash})`,
        );
      ...
      if (props.vmSize && existingVm.hardwareProfile?.vmSize !== props.vmSize) {
        replaceReasons.push(
          `vmSize: ${existingVm.hardwareProfile?.vmSize} → ${props.vmSize}`,
        );
      }
      ...
      const existingPublisher =
        existingVm.storageProfile?.imageReference?.publisher;
      const newImage = props.imageReference || {
        publisher: "Canonical",
        offer: "0001-com-ubuntu-server-jammy",
        sku: "22_04-lts-gen2",
      ...
      if (existingPublisher && existingPublisher !== newImage.publisher) {
        replaceReasons.push(
          `imageReference.publisher: ${existingPublisher} → ${newImage.publisher}`,
        );
      }
      ...
      if (replaceReasons.length > 0) {
        if (!adopt) {
          console.log(
            `[VirtualMachine] VM "${name}" exists with incompatible properties. Replacing due to:\n  - ${replaceReasons.join("\n  - ")}`,
          );
          ...
          await deleteWithAbortVerification(
            ...
            (error) => {
              console.warn(
                `[VirtualMachine] Warning: Could not delete old NIC: ${error}`,
              );
            },
          );
          ...
          await deleteWithAbortVerification(
            ...
            (error) => {
              console.warn(
                `[VirtualMachine] Warning: Could not delete old OS disk: ${error}`,
              );
            },
          );
          ...
          vmParams.networkProfile!.networkInterfaces = [
            { id: newNic.id, primary: true },
          ];
          ...
          console.log(
            `[VirtualMachine] Adopting existing VM "${name}" (adopt: true). Returning existing VM without modifications.`,
          );
          ...
          if (publicIpId) {
            const publicIpResource =
              await clients.network.publicIPAddresses.get(
                resourceGroupName,
    ...
    const result =
      await clients.compute.virtualMachines.beginCreateOrUpdateAndWait(
        resourceGroupName,
        name,
        vmParams,
      );
