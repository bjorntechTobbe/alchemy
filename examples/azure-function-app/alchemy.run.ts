...
console.log(
  `   az functionapp deployment source config-zip --resource-group ${rg.name} --name ${functionApp.name} --src <zip-file>`,
);
...
console.log(
  `   curl "https://${functionApp.defaultHostname}/api/httpTrigger?name=Alchemy"`,
);
console.log(
  `   curl -X POST "https://${functionApp.defaultHostname}/api/httpTrigger" -d "Azure"`,
);

console.log("\n💡 Next Steps:");
console.log(`   1. Deploy your function code using one of the methods above`);
console.log(
  `   2. View logs: az functionapp log tail --name ${functionApp.name} --resource-group ${rg.name}`,
);
